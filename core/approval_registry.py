"""ApprovalRegistry: disk-backed registry for pending order proposals and the
Telegram/Discord/webhook callback parsing that resolves them.

Extracted from vesper/bot/inbound.py (M0-04) so that reading pending
approvals -- trading_mcp/vesper_tools.py's `list_pending_proposals` /
`get_proposal` tools, in particular -- no longer has to import the
`vesper.bot` package to get at this class. Importing `vesper.bot.inbound`
requires Python to first import `vesper/bot/__init__.py`, which eagerly
imports TelegramAdapter/DiscordAdapter/WebhookAdapter/ChannelManager --
none of which a read-only MCP tool listing pending proposals has any
business constructing. This module has no `vesper.bot` dependency at all,
so importing it does not put any of those adapter modules into
`sys.modules`.

`handle_callback_payload()` moved here too, not just the CRUD methods
(register_pending/get_pending/get_decision/list_pending/submit_decision):
it is genuinely shared registry logic, not HTTP-webhook-specific -- it is
called both from vesper/bot/inbound.py's aiohttp routes AND directly from
vesper/bot/telegram_polling.py's outbound long-poll loop (which has no
webhook at all). The two per-user authorization checks it depends on
(`_is_telegram_user_authorized` / `_is_discord_user_authorized`) moved with
it for the same reason. What stayed behind in vesper/bot/inbound.py is
genuinely webhook-specific: the HMAC/Ed25519/bearer-token guards that
authenticate an inbound HTTP request as really coming from Telegram/
Discord/a REST client, and the aiohttp Application/routes themselves.
"""
...
# addition to) vesper/bot/inbound.py's aiohttp webhook route's HMAC/secret-token
# checks. Those prove a request really came from Telegram's servers; they say
