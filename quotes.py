...
class Quotes:
    ...
    def _snapshot(self, symbols: list[str]) -> dict[str, float]:
        try:
            res = self._wb_data.market_data.get_snapshot(
                ",".join(symbols), "US_STOCK", extend_hour_required=True)
            if getattr(res, "status_code", None) != 200:
                raise RuntimeError(f"HTTP {getattr(res, 'status_code', '?')}")
            rows = res.json()
